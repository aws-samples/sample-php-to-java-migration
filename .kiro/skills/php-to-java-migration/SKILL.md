...
**Plain/procedural PHP (no framework, no ORM)** — needs a different discovery playbook:
- Classify every `.php` file before anything else: **API** (reads `$_GET`/`$_POST`/`$_REQUEST`/`$_SERVER['REQUEST_METHOD']` or sets `header('Content-Type')`), **BATCH** (no HTTP superglobals; uses `$argv`, `exit(0)`, or has a cron comment — maps to `@Scheduled`/`CommandLineRunner`, *not* a controller), **SHARED** (only class/function definitions, included by others), **CONFIG** (only assignments: `$config[`, `define(`).
- Infer routes from file paths + method detection: `api/traspasos/buscar.php` handling POST → `POST /api/v1/traspasos/buscar`. Request params from superglobal reads; response DTO shape from `json_encode(...)` payloads.
- Infer entities from SQL, not models: extract every SQL string (`mysqli_query`, `ifx_query`, `odbc_exec`, `$db->query`, any `SELECT`/`INSERT INTO`/`UPDATE`/`DELETE FROM` literal), harvest table names, column lists, and JOIN conditions (→ FK relationships); infer column types from context (`intval()` → integer, date functions → date). This replaces the `php-model-discovery` manifest when there is no ORM to scan.
- Record the derived names in a **naming contract** (table→entity, column→field, file→endpoint) that later phases treat as immutable input — never invent a name that isn't derived from source.
...
3. **Failure-hardening pre-scan** — run before deeper analysis; every finding goes in `MIGRATION_ASSESSMENT.md` (validated at 10-repo scale by the internal ATX PHP→Spring Boot pipeline):
   - **Dangerous patterns** (files containing these are never auto-translated — each gets a manual-review entry): `eval(`, `system(`, `exec(`, `shell_exec(`, `passthru(`, `preg_replace('/e'`, `create_function(`, `include $_GET[...]`/dynamic include of user input, `extract($_POST)`.
   - **Mega-files** (>5000 lines): flag and split by concern *before* translation — a single pass over a mega-file saturates agent context and produces silently incomplete output. Same at repo scale: **>150–200 PHP files must be split into bounded contexts and migrated as separate parallel runs**; a single run over an XL repo yields <50% coverage that *looks* finished.
   - **Circular includes**: build the include/require graph, run cycle detection; files in a cycle are treated as shared config to absorb, not endpoints to translate.
   - **Unparseable files** (syntax errors, non-UTF-8 encoding, binary): flag and continue — never let one broken legacy file abort discovery.
   - **Sensitive-data columns**: scan SQL/models for PII column names (national IDs, SSN/CURP/NSS-style fields, `password`, card/account numbers). Mark each `sensitive: true` in the inventory — Phase 5/6 must give every one an explicit decision: validation constraint on input DTOs, masking in logs, omission from response DTOs, and (for reversible PII, never passwords) an encryption converter. Passwords get hashing (BCrypt), not encryption.
   - **SQL-injection sites** (string-concatenated SQL): record them — the Java side must be parameterized everywhere, and each site is a place where PHP's actual runtime behavior (what malformed input *did*) may need a bug-for-bug decision.
4. Rank modules by fan-in. Leaf modules (zero internal dependents' dependencies) migrate first; high-fan-in modules last.
5. Choose the cutover mechanism at the gate:
...
6. Any JPA `@OneToMany`/`@ManyToMany` reachable from a serialized REST response needs an explicit
   ...
   **Refinement (server-rendered apps)**: the rule above is for JSON APIs. When the target renders
   server-side templates (Twig → Thymeleaf) and templates walk lazy relations during rendering —
   exactly as Twig did — `open-in-view: true` *is* the parity-faithful setting: it reproduces
   Doctrine's request-long entity manager. Choose per app: API-style → OSIV off + DTO projections;
   template-rendered port preserving Twig-era lazy access → OSIV on, documented in
   `BEHAVIOR_CHANGES.md`. (Surfaced by the Symfony Demo migration: every blog page 500'd on
   `post.tags` until OSIV matched the source app's rendering lifecycle.)
7. Verify how the JDBC driver *binds* date/time parameters against how the schema *stores* them —
   they can disagree silently. Doctrine stores datetimes as TEXT `Y-m-d H:i:s`; the xerial SQLite
   JDBC driver defaults to binding `LocalDateTime` as epoch-millis INTEGER. In SQLite, TEXT
   compares greater than any INTEGER, so a `published_at <= :now` filter matches zero rows — no
   error, just empty results. Fix at the datasource (e.g. `date_class=text` +
   `date_string_format=yyyy-MM-dd HH:mm:ss`) so comparisons stay lexicographic and
   Doctrine-compatible. Generalize: for every DB, confirm one round-trip WHERE-on-datetime query
   returns seeded rows before declaring the data layer done.
...
| Share-nothing request model | PHP resets *all* state per request — a file-level `$errors = []` or accumulating array is safe by construction. A Spring `@Service`/`@Controller` is a **singleton shared across all concurrent requests**: the same variable translated as a mutable instance field is a race condition that no unit test catches. | NEVER translate PHP file/function-level mutable state into instance fields on Spring beans. All per-request accumulation becomes method-local variables (or request-scoped beans when unavoidable). Reviewers grep for mutable `List`/`Map`/`StringBuilder`/counter fields on `@Service`/`@Controller` classes — any hit is a finding. |
| `die("msg")` / `exit(1)` | terminates the PHP process — per-request, that's just an error response | `System.exit()` kills the whole JVM and every in-flight request. Map `die(msg)` → throw a business exception handled by `@ControllerAdvice`; `exit(0)` at the natural end of a CLI script → plain method return. Assert `System.exit` never appears in translated code. |
...
6. **Audit completeness with a traceability matrix** — parity proves what exists behaves the same; this proves nothing was silently dropped. Classify **every** PHP source file into exactly one status and emit `TRACEABILITY_MATRIX.md`:

   | Status | Meaning |
   |---|---|
   | CONVERTED | maps to named Java artifact(s) — cite them |
   | ALREADY_HANDLED | infra absorbed elsewhere (config → `application.yml`, session helper → Spring Security) — cite what replaced it |
   | SKIPPED | deliberately not migrated (test file, dev tool, vendor) — with rationale |
   | MISSING | endpoint/business-logic file with **no** Java counterpart — the actionable gap list |
   | FLAGGED | partially converted (e.g. dangerous-pattern file) — document what was left behind |

   Effective coverage = (CONVERTED + ALREADY_HANDLED) / (total − SKIPPED). Anything below 100% with unexplained MISSING rows blocks cutover. This audit is read-only and cheap; run it by a fresh-context reviewer, not the agent that did the translation.
7. **Cut over**: one route/module at a time (strangler) or flip the proxy (big-bang). Keep PHP hot for rollback for one release cycle.

**Deliverable**: parity report per module + `TRACEABILITY_MATRIX.md` + cutover checklist.
...
- Dangerous-pattern files (`eval`, `system`/`exec`/`shell_exec`, dynamic includes of user input) are never auto-translated — flagged for manual review, always.
- XL scope is split, not brute-forced: files >5000 lines are split by concern and repos >150–200 PHP files are split into bounded contexts before fan-out. Context saturation produces silently incomplete output that looks finished.
- No mutable per-request state on Spring singletons; no `System.exit()` anywhere in translated code.
- Every `sensitive: true` field carries an explicit decision (input validation, log masking, response omission, hash-vs-encrypt) — omission is a review finding, not a default.
- Cutover requires both referees green: behavioral parity (the judge) *and* file-level completeness (`TRACEABILITY_MATRIX.md` with zero unexplained MISSING).
...
Procedural-PHP discovery playbook, failure-hardening pre-scan, share-nothing/`System.exit` traps,
sensitive-field discipline, traceability matrix, and XL context-saturation limits informed by an
earlier internal PHP-to-Spring-Boot transformation pipeline benchmarked across a portfolio of real
procedural-PHP and CodeIgniter repositories. Its three-stage discover → generate → validate
structure with a machine-readable naming contract between stages independently corroborates this
skill's judge-first, contract-driven approach.
