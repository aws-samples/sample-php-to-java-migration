...
- **Doctrine (XML mapping)**: Sylius (`Sylius/Sylius`, MIT) — 98 entities/mapped-superclasses found
  ...
  — see Rules below. It also surfaced a `DOCTRINE_TYPE_MAP` case-sensitivity bug: the map's keys
  were uppercase (`'INTEGER'`), matching how a `Types::INTEGER` constant reference happens to get
  captured (the constant's *name*, as literal source text), but Doctrine's actual DBAL type
  identifiers — and any `type: 'integer'` literal-string column declaration — are lowercase. Fixed
  by lowercasing the map's keys and normalizing the lookup key via `strtolower()` in
  `mapDoctrineType()`, so both capture paths resolve to the same entry. Caught 9 false
  "unmapped type" flags in Sylius's attribute-mapped entities; re-verified against Symfony Demo
  afterward to confirm no regression on the previously-correct mappings there.
